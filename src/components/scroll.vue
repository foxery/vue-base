...
export default {
  ...
  created: function() {
    let _this = this;
    if (
      (this.contentArr != null && this.contentArr.length < this.lineNum) ||
      (this.dLength != null && this.dLength < this.lineNum)
    ) {
  ...
  mounted: function() {
    ...
    if (this.dLength !== null) {
      for (let i = 0; i < this.dLength; i++) {
        this.$el.childNodes[0].appendChild(
          this.$slots.slide[i].elm.cloneNode(true)
        );
      }
    }
  }
